# Slice 1 phase 3 — WV1018736 activation chain

**Started:** 2026-07-03T14:50:42.716Z  
**Finished:** 2026-07-03T14:51:39.230Z  
...
| Check | Result |
|-------|--------|
| `has_permit_limit` increased | 0 (2893 → 2893) |
| Pending `missing_internal` dropped | 143601 → 143433 (Δ -168) |
| WV1018736 off top gap list | WV1018736 was not on top gap list before or after |
| Scoped detect | Ran slice3-echo-batch-detect --permit WV1018736 |
...
| Has permit_limit | 2893 | 2893 | 0 |
| Mirror keys | 15611 | 15667 | 56 |

## Commands run

```bash
npm run qa:slice1-activation-gaps -- --suffix before
npm run qa:slice1-seed-exceedances -- --limit 50 --batches 20 --permit WV1018736
npm run qa:slice1-repair-stuck-keys -- --limit 50 --batches 10 --permit WV1018736
npm run qa:slice1-reconcile -- --limit 10000 --batches 10
npm run qa:slice1-activation-gaps -- --suffix after
npm run qa:slice3-echo-batch-detect -- --permit WV1018736 --wait 20
```
