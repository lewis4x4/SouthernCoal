# Slice 1 phase 3 — WV1006304 activation chain

**Started:** 2026-07-03T15:07:21.991Z
**Finished:** 2026-07-03T15:08:18.672Z
...
| Check | Result |
|-------|--------|
| `has_permit_limit` increased | 0 (2970 → 2970) |
| Pending `missing_internal` dropped | 143295 → 143178 (Δ -117) |
| WV1006304 off top gap list | WV1006304 was not on top gap list before or after |
| Scoped detect | Ran slice3-echo-batch-detect --permit WV1006304 |
...
| Has permit_limit | 2970 | 2970 | 0 |
| Mirror keys | 15713 | 15753 | 40 |

## Commands run

```bash
npm run qa:slice1-activation-gaps -- --suffix before
npm run qa:slice1-seed-exceedances -- --limit 50 --batches 20 --permit WV1006304
npm run qa:slice1-repair-stuck-keys -- --limit 50 --batches 10 --permit WV1006304
npm run qa:slice1-reconcile -- --limit 10000 --batches 10
npm run qa:slice1-activation-gaps -- --suffix after
npm run qa:slice3-echo-batch-detect -- --permit WV1006304 --wait 20
```
