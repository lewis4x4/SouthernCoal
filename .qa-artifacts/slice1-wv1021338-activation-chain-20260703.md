# Slice 1 phase 3 — WV1021338 activation chain

**Started:** 2026-07-03T14:36:00.866Z  
**Finished:** 2026-07-03T14:36:57.223Z  
...
| Check | Result |
|-------|--------|
| `has_permit_limit` increased | 0 (2803 → 2803) |
| Pending `missing_internal` dropped | 144060 → 143769 (Δ -291) |
| WV1021338 off top gap list | WV1021338 was not on top gap list before or after |
| Scoped detect | Ran slice3-echo-batch-detect --permit WV1021338 |
...
| Has permit_limit | 2803 | 2803 | 0 |
| Mirror keys | 15458 | 15555 | 97 |

## Commands run

```bash
npm run qa:slice1-activation-gaps -- --suffix before
npm run qa:slice1-seed-exceedances -- --limit 250 --batches 10 --permit WV1021338
npm run qa:slice1-repair-stuck-keys -- --limit 100 --batches 10 --permit WV1021338
npm run qa:slice1-reconcile -- --limit 10000 --batches 10
npm run qa:slice1-activation-gaps -- --suffix after
npm run qa:slice3-echo-batch-detect -- --permit WV1021338 --wait 20
```
