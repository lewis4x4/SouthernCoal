-- Slice 5 CMS schema: sample_date, sampling_calendar, reporting_period_start, closed_date
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE table_schema = 'public' AND table_name = 'dmr_submissions' AND column_name = 'reporting_period_start'
  ) INTO v_cms_dmr;

  SELECT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'sampling_calendar' AND column_name = 'scheduled_date'
  ) INTO v_use_sampling_calendar;

  SELECT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'sampling_events' AND column_name = 'sample_date'
  ) INTO v_use_sample_date;

  SELECT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'corrective_actions' AND column_name = 'closed_date'
  ) INTO v_ca_closed_date;

  SELECT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'sites' AND column_name = 'state_code'
  ) INTO v_sites_state_code;
  ...
  ELSIF v_use_sampling_calendar THEN
    SELECT
      COUNT(*),
      COUNT(*) FILTER (
        WHERE sc.sampling_event_id IS NOT NULL
          OR sc.status IN ('completed', 'sampled')
      )
    INTO v_sampling_due, v_sampling_completed
    FROM public.sampling_calendar sc
    JOIN public.outfalls o ON o.id = sc.outfall_id
    JOIN public.npdes_permits p ON p.id = o.permit_id
    WHERE p.organization_id = p_org_id
      AND public.in_live_program_scope(p_org_id, p.site_id, p.id, o.id)
      AND sc.scheduled_date >= p_snapshot_date - INTERVAL '30 days'
      AND sc.scheduled_date <= p_snapshot_date
    ...
    EXECUTE $state$
      ...
      FROM (
        ...
        LEFT JOIN public.npdes_permits p
          ...
          AND public.in_live_program_scope($1, s.id, p.id, NULL)
        LEFT JOIN public.outfalls o
          ON o.permit_id = p.id
          AND public.in_live_program_scope($1, s.id, p.id, o.id)
        LEFT JOIN public.exceedances e
          ON e.outfall_id = o.id
          AND e.organization_id = $1
        LEFT JOIN public.compliance_violations cv
          ON cv.site_id = s.id
          AND cv.organization_id = $1
          AND public.in_live_program_scope($1, s.id, NULL, NULL)
