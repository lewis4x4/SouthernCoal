    ...
    >
      ...
      <div className="flex items-center gap-1 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
        {entry.status === 'queued' && canProcessQueueEntry(entry) && (
          <button
            onClick={() => can('process') && processEntry(entry.id)}
            disabled={!can('process')}
            className="px-2.5 py-1 text-[11px] font-medium rounded-md bg-status-imported/15 text-status-imported border border-status-imported/20 hover:bg-status-imported/25 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            title={can('process') ? `Process (${parserRoute.label})` : 'Permission required to process files'}
          >
            <Play size={10} className="inline mr-1" />
            Process
          </button>
        )}
        {entry.status === 'failed' && canProcessQueueEntry(entry) && (
          <button
            onClick={() => can('retry') && retryFailed(entry.id)}
            disabled={!can('retry')}
            className="px-2.5 py-1 text-[11px] font-medium rounded-md bg-status-processing/15 text-status-processing border border-status-processing/20 hover:bg-status-processing/25 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            title={can('retry') ? 'Retry processing' : 'Permission required to retry failed files'}
          >
