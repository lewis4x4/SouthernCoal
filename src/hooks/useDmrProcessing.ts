...
/**
 * NetDMR bundle processing — triggers parse-netdmr-bundle Edge Function.
 * Handles dmr category ZIP archives and single CSV exports.
 */
export function useDmrProcessing() {
  const { log } = useAuditLog();

  const processDmr = useCallback(async (queueId: string) => {
    ...
    try {
      ...
      try {
        const { error: fnError } = await Promise.race([
          supabase.functions.invoke('parse-netdmr-bundle', {
      ...
      toast.info(`NetDMR processing started for ${entry.file_name}`);
    } catch (err) {
  ...
  const processAllQueuedDmrs = useCallback(async () => {
    const entries = useQueueStore.getState().entries;
    const queued = entries.filter(
      (e) => e.status === 'queued' && e.file_category === 'dmr',
    );

    if (queued.length === 0) {
      toast.info('No queued DMR files to process.');
      return;
    }
    ...
    toast.info(`Processing ${queued.length} DMR file${queued.length > 1 ? 's' : ''}...`);
    ...
    for (let i = 0; i < queued.length; i++) {
      ...
      try {
        ...
        if (fnError) {
          console.error(`[dmr] Failed ${entry.file_name}:`, fnError.message);
        ...
        if (err instanceof DOMException && err.name === 'AbortError') {
          ...
          console.error(`[dmr] Error ${entry.file_name}:`, err);
    ...
    toast.success(`Finished processing ${queued.length} DMR files.`);
  ...
  const retryFailed = useCallback(
    async (queueId: string) => {
      await processDmr(queueId);
    },
    [processDmr],
  );

  return { processDmr, processAllQueuedDmrs, retryFailed };
}
