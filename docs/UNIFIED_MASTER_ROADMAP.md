...
## 8. Counterparty & Interaction Graph (added v1.2)

The base **party/relationship/interaction layer** every management system stands on — the "who the players are and every time we touched them" spine SCC-OS never had (the keystone memo's admission-ledger, Regulator's Mirror, and disclosure engine all silently assume it). Full design, adversarially reviewed and schema-verified: `docs/HANDOFF_COUNTERPARTY_GRAPH.md`.

**Two-slice split (the load-bearing decision):**
- **Slice A — Party Spine (build now, parallel to ECHO, fully autonomous):** `parties` / `party_role_types` / `party_relationship_types` / `party_roles` / `party_relationships` / `party_merge_events` + the `organizations.party_id` bridge. Folds in `user_profiles`, the 27 orgs, `emergency_contacts`, state agencies, labs, and the named discovery owners. `owns_or_controls` edges derive from `msha_mine_org_map` (one source of truth). Reuses `get_user_org_id()`, `set_updated_at()`, the §7.2 bitemporal triad, and the `human_overrides` append-only pattern verbatim. No privilege surface → no gate.
- **Slice B — Interaction Ledger (governance-first):** append-only, evidence-grade touch record; privilege **fail-closed** and mapped into the existing `src/lib/classification.ts` floor model (no competing enum); `interactions` always tenant-private even when referencing a shared regulator party.

**New hard gate (mirrored in ENGINEERING_FREEDOM v1.3):** interaction-ledger **auto-capture** (`email_sync`/`portal_scrape`/`field_voice`) must not write a row until counsel signs a versioned privilege + retention + FOIA-response policy. Slice A and manual Slice B capture are ungated. The cross-tenant **Adjudicator Atlas** is deferred and counsel-gated (k≥5 suppression). Sequence Slice A before `job_runs`/`alert_acknowledgments` is fine — no collision (both confirmed absent).
